...
bool VerifyCRC(const Packet& packet)
{
	...
	std::memcpy(&received_crc, packet.Payload + packet.Size, sizeof(received_crc));

	std::uint32_t calculated_crc = CRC::Calculate(packet.Payload, packet.Size, CRC::CRC_32());
	...
	printf("Waiting...\n");
	{
		...
		while (packet.Type != PacketType::End)
		{
			...
			VerifyCRC(packet);
				if (!VerifyCRC(packet))
				{
					printf("CRC check failed for received packet!\n");
